function [fn, overalLSL, nLessThan20, nCalls, lateCalls, lastCalls, pickedCalls] = MultiSkill(x, runlength, seed, nCallTypes, nAgentGroups, arrivalRates, R, Route, shifts)
...
lateCalls = []; % calls that are picked up late (>20s) or expired without being picked up
lastCalls = []; % calls that are picked up by an agent who is the last available one in the group since then
% NJ: it would be nice to include all calls picked up by the agent, but
% would require keeping track of an entire history of who picked up which call.
lastAgent = -1 * ones(nAgentGroups, size(shifts, 1)); % the index of the agent in each group * shift

% pickedCalls(i,j) = callType if in period i agent j picked up a call of
% this callType
pickedCalls = sparse(36, sum(sum(x)));
nShifts = size(shifts, 1);

%totalPicked = 0; % for debug purpose
                        ...
                        if nAvailable(nextGroup) > 0 % if there's available agent in that group
                            ...
                            for j = 1+sum(nPerGroup(1:nextGroup-1)):sum(nPerGroup(1:nextGroup)) % all agents in nextGroup
                                if( Agents(j,4) <= time && Agents(j,3) == 1 ) % next available time is now and agent is available
                                    pickedCalls(curShift, j) = callType; % record call
                                    ...
                                    if lastAgent(nextGroup, Agents(j,2)) == -1 && nAvailable(nextGroup) == 0 % when this is the last available agent in the group
                                        lastAgent(nextGroup, Agents(j,2)) = j; % label this agent
                                        lastCalls = [lastCalls, [Events(2:4,nextEvent); nextGroup]]; % record this call
                                    end
                ...
                if Agents(a,4) == time % next availability is now
                    ...
                    while j <= length(callQueue(1,:)) && callTaken == 0
                        if callQueue(3,j) < time % call expiration time
                            lateCalls = [lateCalls, [callQueue(:,j); 0]];
                            ...
                            if (R(Agents(a,1),callQueue(2,j)) ~= 1000) % take first feasible call in queue
                                ...
                                if( time - callQueue(1,j) <= SLtime/60 )
                                    ...
                                    lateCalls = [lateCalls, [callQueue(:,j); 1]];
                                end
                    ...
                    if (callQueue(3,j) > time) % not expired yet
                        ...
                        if minimum ~= 1000 % any feasible agent in agentNumbers
                            ...
                            if( time - callQueue(1,j) <= SLtime/60 ) % time answered within 20s
                                ...
                                lateCalls = [lateCalls, [callQueue(:,j); 1]]; % call is late
                            end
                            %Generating service time of agent
                            RandStream.setGlobalStream(ServiceStream);
                            sTime = exprnd(meanST);
                            a = agentNumbers(minAgent); % pick the agent who is most preferred by the call
                            pickedCalls(1+floor(callQueue(1,j)/15), a) = callType; % record call
                            Agents(a,3) = 0;
                            Agents(a,4) = time + sTime; % next available time
                            nAvailable(Agents(a,1)) = nAvailable(Agents(a,1)) - 1;
                            if lastAgent(Agents(a,1),Agents(a,2)) == -1 && nAvailable(Agents(a,1)) == 0 % when this is the last available agent in the group
                                lastAgent(Agents(a,1),Agents(a,2)) = a; % label this agent
                                lastCalls = [lastCalls, [callQueue(:,j); Agents(a,1)]]; % record this call
                            elseif a == lastAgent(Agents(a,1),Agents(a,2)) % when this is the last available agent in the group
                                lastCalls = [lastCalls, [callQueue(:,j); Agents(a,1)]]; % record this call
                            end
...
%                             totalPicked = totalPicked + 1;
    ...
    overalLSL = sum(sum(nLessThan20)) / sum(sum(nCalls));
       
    fn = CostPerDay( Agents, R, shifts );
